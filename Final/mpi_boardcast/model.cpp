...
Model::update()
{
    ...
    // 必须用vector，因为map的迭代器不是线程安全的
    std::vector<std::unordered_map<std::size_t, std::uint8_t>> currthread_nextfront(get_num); 
    ...
    m_total_time += time;
    m_time_step += 1;
    std::cout << "Avg time elapsed:" << m_total_time/m_time_step << std::endl;
