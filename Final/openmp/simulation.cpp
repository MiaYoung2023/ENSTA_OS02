...
int main(int nargs, char* args[]) {
    ...
    while (simu.update()) {
        if (rank == 0) {
            ...
            std::vector<uint8_t> recv_buffer(fire_map.size());

            for (int i = 1; i < size; i++) {
                MPI_Recv(recv_buffer.data(), recv_buffer.size(), MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                for (size_t j = 0; j < fire_map.size(); j++) {
                    fire_map[j] = std::max(fire_map[j], recv_buffer[j]); // 合并火势数据
                }
            }

            // 渲染模拟
            displayer->update(simu.vegetal_map(), fire_map);
            ...
            while (SDL_PollEvent(&event)) {
                if (event.type == SDL_QUIT || 
                    (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE)) {
                        ...
                        << std::chrono::duration_cast<std::chrono::milliseconds>(end_sim - start_sim).count() 
                        << " ms" << std::endl;
                        return EXIT_SUCCESS;
                }
            }
            ...
            // 其他进程计算火势并发送数据
            std::vector<uint8_t> fire_map = simu.fire_map();
            MPI_Send(fire_map.data(), fire_map.size(), MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
        }
